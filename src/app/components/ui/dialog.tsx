// shadcn/ui Dialog(new-york 风格)的水墨适配底件:Radix 行为 + 本项目 token 皮。
// 由 components.json + `bunx shadcn add dialog` 的拷贝管线维护(已按水墨主题改皮,
// 后续 CLI 更新组件时 diff 这里的改皮差异)。简单确认框/纯表单弹层直接用本文件;
// 器物级自定义皮肤(如游戏卷轴 ScrollShell)直接用 radix-ui 的 Dialog 原语自行组皮,
// 行为口径(焦点陷阱/aria-modal/Esc/点外关闭)与本底件保持一致。
