// 令笺(#237 一期 T2):军师幕里名将主动技的牌面变体——方案 §4 P0-B「技=令笺变体」,
...
// ——选中/确认都是消费方(DecisionScrolls.JinnangScroll)的普通按钮点击,本件只收
// className(选中态 sel / 灰置态 off 挂根,复用 jinnang-card.css 的交互态)。
//
// 数据单源:文案随 choices 载荷过网(UI 不回查名将目录,与锦囊牌「牌面回查目录、
// 技能文案走载荷」的口径一致,见 JinnangScroll 注)——hero/name/text 由调用方从
...
export function JinnangLingjian({ hero, name, text, className, children, ...rest }: JinnangLingjianProps) {
  return (
    ...
    // 差量样式(金界格线/技章/徽记/摘要区)收口在 scroll.css 的 .jn-lingjian 作用域。
