...
export function DecisionScrollLayer({
  ...
  const child = (() => {
          ...
          if (propDef) {
            return (
              <TreasureVisitorScroll
                ...
                property={{
                  id: propDef.id,
                  tradeAdd: propDef.tradeAdd,
                  tradeMult: propDef.tradeMult,
                }}
    ...
    if (
      ...
      snapshot.turnPhase === "AwaitingExhaustion"
    ) {
    ...
    if (
      snapshot.turnPhase === "AwaitingHeroPick" &&
      snapshot.offeredHeroes.length > 0 &&
      interactive
    ) {
