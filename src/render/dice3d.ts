...
// WebGL 不可用时降级为 no-op(available=false),让上层 animate.ts 走旧 textContent 切换 fallback。
// core 层零 DOM;本文件纯属 render 层(不违反 core/player-agnostic 红线)。
...
export class ThreeDice {
  ...
  /** 静息姿态:die 面朝上,中心贴地,不动画。用于 fullRender 初始化展示。 */
