...
回合制格子桌游,2–8 人(单机:1 真人对阵电脑;或联机:每人一台设备)。玩家开局选都城(取代起点),通过掷骰行军、购地经营、拼点探宝、珍宝交涉积累现金;**身价 = 仅现金**,率先达到目标身价或群雄尽灭即称帝。完整规则见 **[规则手册](./docs/reference/rules/README.md)**。

## 机遇系统配置

落格触发的「机遇」事件分好运/中性/霉运三档,默认档位在 `public/config/jiyu.json`:`triggerRate` 为触发概率百分比(默认 40),`baseRates` 的 `good / neutral / bad` 为三档基准(默认 30/45/25)。三档按占比归一,**和不必为 100**(如 10/10/10 即各 1/3);任一档 ≤0 或非法时引擎整体回退默认 30/45/25。单机开局前可在「起兵」屏的「机遇」折叠区对本局覆盖这四个值(默认值即读自该文件),仅影响当局。
