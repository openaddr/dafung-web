...
**声望(Reputation)**:
每位 Seat 的对局内数值(-100~+100,开局恒 0,预留非 0 配置口子),决定奇遇抽取时好运/中性/霉运三档的真实比例;只被奇遇内的抉择与结果改变,不随回合自然波动。
...
**档位(Fortune Tag)**:
奇遇的三分定性——好运(纯正收益)/中性(有得有失需权衡)/霉运(纯负收益)。三档基准概率可配置,配置值之和不必为 100(按配置值占比归一为真实概率)。
