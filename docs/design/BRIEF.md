...
换皮测试结论:纸-墨-印的语言若换到唐/宋仍成立,但**三国特异性由内容器物承担**——国号方印(魏蜀吴…)、城池匾牌、委任状/都/起/胜的功能印系、名将画像、IE 校验过的三大地图城名;印章系统与游戏机制(委任状=资源、国号=身份)互证,不是贴纸。
...
| #   | 屏                   | 验收要点                                                                                               | 状态            | 包       |
| --- | -------------------- | ------------------------------------------------------------------------------------------------------ | --------------- | -------- |
| 1   | 对局内·棋盘/演出     | 山水带/江河/驿道提墨/宣告动效(ADR-0015)无回退                                                          | 绿(评审放行 R1) | D        |
| 2   | 对局内·HUD/侧栏/手牌 | 笺头四区、方印、墨钮行军、截断收口、演出窗口径                                                         | 绿(评审放行 R1) | D        |
| 3   | 对局内·卷轴/终局     | 挂轴/题印/两态破产+尚欠小字/胜利屏满屏/交涉 Sym                                                        | 绿(评审放行 R1) | D        |
| 4   | 首页                 | 山水横带+落款印+签条入口;MapSelectPanel 弹层协调                                                       | 绿(评审放行 R2) | A+E      |
| 5   | 单机配置             | 手札卡+钤印字盘                                                                                        | 绿(基线)        | —        |
| 6   | 联机大厅             | 座次笺/房码签条/控件种/建入笺头/虚位浅印/全角标点                                                      | 绿(评审放行 R1) | C        |
| 7   | 地图编辑器           | 工具钮/表单/侧栏全入控件种;InputScroll 协调                                                            | 绿(评审放行 R2) | B        |
| 8   | 共享组件/全局        | MapSelectPanel(已回炉)、HintBar/ConnectionBanner/ConfirmDialog 字族迁 wenkai、theme.success 加深(主线) | 绿(评审放行 R2) | A+E+主线 |
