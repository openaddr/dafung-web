...
从三国杀 wiki(patchwiki 图床)扩一名名将入库,共两步:找页面 id → 跑一条命令。图片落库后,
在 `src/core/heroes.ts` 补一条 `HeroDef` 数据行即完成"新名将可玩"。

## 管线用法(两步)

### 第 1 步:在 wiki 上找到名将「经典形象」的 File 页 id(curid)

1. 打开名将 wiki 页(如 `https://wiki.biligame.com/sgs/司马懿`),点其立绘进入 **「文件:XXX-经典形象.png」**
   的 File 页(File 页 = `ns=6` 的「文件:」命名空间页,注意不是名将主页面)。
...
`--id` 命名规范:`hero:<py>:sgs`,其中 `<py>` 是名将名全拼(小写,如 `zhouyu`/`caopi`/`zhangxingcai`/`simayi`)。
...
**授权口径**:三国杀官方名将原画(游卡桌游)版权归官方,本项目**仅限学习与朋友间娱乐,不商用**;
...
`image` 指向落盘路径(`/assets/heroes/hero-<py>-sgs.png`),招贤卷轴画像位
(`src/app/screens/game/scroll/HeroPickScroll.tsx`)会自动读取 `HeroDef.image` 展示
(获得后仪表条「名将」徽章可展开明细查看):
...
- **传 File 页,别传名将主页面**:主页面(ns=0)没有 imageinfo,管线直接抛错。
- 直链模式拿不到 wiki 上传者(manifest author 退化为「三国杀官方名将原画(游卡桌游)」),入库优先用 File 页 URL。
