...
| 玩法事件 | `roll` 掷骰(点数/步数/加成)· `buy` 购地 · `upgrade` 扩军/成交升级 · `trade` 珍宝交涉 + escrow 交割/退回 · `supply` 补给/委任状 · `tax` 税关 · `branch` 辅路抉择/中伏跳过 · `halt` 驻跸必停 · `setup` 开局流程(定序/**三候选生成**/选都/招贤)· `skill` 时机技能击发(26 时机)与名将主动技发动(`heroSkill`/`heroRelief`/`heroPatronage`/`heroWarDrum`)· `system` 其余玩法杂项(随机事件含事件 id/商市/破产清算/警告)· `victory` 胜负 | `src/core/game.ts` 各结算点 |
