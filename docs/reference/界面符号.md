...
> ⚠ 渲染口径(2026-09 视觉重做):符号一律经 `Sym` 组件
> (src/app/screens/shared/Sym.tsx)以内联 SVG 渲染——← ↶ ↷ ◎ ♪ 等字形不在
> 离线中文字体子集里,字符裸排会出豆腐块(实拍翻车案例:大厅「返回■首页」、
> 编辑器「■撤销」)。新用途先在本表登记,再在 Sym 的 PATHS 里加 path。
