...
describe("默认配置文件 public/config/jiyu.json(#125)", () => {
  ...
  it("数值口径合法:触发 0~100、三档为正数", () => {
    ...
    for (const v of [
      jiyuConfig.baseRates.good,
      jiyuConfig.baseRates.neutral,
      jiyuConfig.baseRates.bad,
    ]) {
...
describe("设置屏纯函数(#125:表单 → EngineConfig.encounter)", () => {
  ...
  it("sanitizeEncounterForm:触发夹紧 0~100、三档夹紧 ≥0、非数记下界 0", () => {
    expect(
      sanitizeEncounterForm({ triggerRate: 150, good: -3, neutral: 0, bad: Number.NaN }),
    ).toEqual({
      triggerRate: 100,
      good: 0,
      neutral: 0,
  ...
  it("parseEncounterDefaults:结构齐全且为有限数才接受,缺字段/类型错 → null", () => {
    expect(
      parseEncounterDefaults({ triggerRate: 50, baseRates: { good: 1, neutral: 2, bad: 3 } }),
    ).toEqual({ triggerRate: 50, good: 1, neutral: 2, bad: 3 });
    expect(
      parseEncounterDefaults({ triggerRate: "40", baseRates: { good: 1, neutral: 2, bad: 3 } }),
    ).toBeNull();
    expect(parseEncounterDefaults({ baseRates: { good: 1, neutral: 2, bad: 3 } })).toBeNull();
    expect(
      parseEncounterDefaults({ triggerRate: 40, baseRates: { good: 1, neutral: 2 } }),
    ).toBeNull();
    expect(
      parseEncounterDefaults({
        triggerRate: 40,
        baseRates: { good: 1, neutral: 2, bad: Infinity },
      }),
    ).toBeNull();
