...
- [ ] 为什么联机模式端口和正常启动dev端口不一样, 这影响我们构建后的真实程序吗? 对真实用户有影响吗?
- [ ] 为什么联机模式开始游戏, 没有选择都城的阶段, 直接就落定了都城了?
- [ ] 联机模式下, 刚移动就弹出结算框, 比如落地是个未购买的城池, 还没播放完移动动画就已经弹出是否购买的窗口了, 这样看上去时序不对
