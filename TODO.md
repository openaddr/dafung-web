...
- [ ] 地图边缘超出地图的部分, UI看上去应该和地图是连续的, 也就是看上去地图和外部不是这种割裂的感觉
- [ ] 城池的视觉效果可以看起来更大一点, 现在看感觉还是能再放大一些的, 城池目前太小啦看不清
- [ ] 点击行军的音效换一下, 行军是一个频繁触发的行为, 应该是很浅的, 轻快的音效, 甚至哪怕没有音效都不能太吵
- [ ] 开局选择都城时, 有些不能选择的地方, 点击了没有任何响应, 交互感差
- [ ] 允许用户在未进入游戏的首页 设置自己的国号, 之后进入游戏默认就是这个国号, 联机模式, 如果遇到和自己国号相同的, 则可以以 东西南北这样的前缀来区分 , 比如 宁, 北宁, 南宁, 东宁, 西宁, 后宁, 大宁, 这只有7个, 帮忙在想一个凑够8个
- [ ] 游戏最大支持玩家数量扩展到8人
- [ ] 玩家旗子有点不明显, 甚至移动的时候看不太清人在哪, 可以考虑优化下这方面的视觉效果, 比如再放大点旗子(或者更好的优化效果)
- [ ] 购地的时候, 我发现你给出的地属性有问题啊, 首先应该只有3个等级, 为啥有五个等级 (每个等级的属性也应该在json中写过了吧), 第二 为什么会有过路费和升级费的说法? 游戏没有过路费, 纯靠卖珍宝, 也没有升级费, 只能靠其他玩家到达这里免费升级或者自己到达城池免费升级
- [ ] 手牌区的元素比如按钮, 样式大小七扭八歪的, 很难看, 甚至大小都不统一, 作为专业UI设计者, 这块好好设计实现下
- [ ] 每个城池都可以单击查看详情, 包括 特殊地点
- [ ] 城池打开后的查看详情页, 中间有个关闭用的x去掉, 直接点击空白关闭已经成为用户常识了
- [ ] 游戏开始时, 不能被选中做都城的城池应该有些灰化的视觉效果, 让用户直观知道这些城池不能选;  可以选择的城池, 第一次单击时是查看城池详情, 在详情内有按钮可以确定或者取消选中作为都城
- [ ] 珍宝和英雄在手牌中点击能查看详情, 查看详情的地方预留出来显示图片的地方, 这个图片用来显示珍宝或者英雄的图片; 英雄的图片你可以自行去互联网上获取三国杀这个游戏中武将的画像作为图片,不过要注意图片的比例要保持一致, 当前游戏为学习使用, 仅供朋友间娱乐, 不商用, 可放心从互联网获取任何资源使用
