...
- 两条豁免:①纯视觉/器物件(棋盘 SVG、卷轴装饰、画像卡)不在此列;②器物级自定义皮肤(如游戏卷轴 `ScrollShell`)可直接用无头原语自行组皮,但行为口径(焦点陷阱/Esc/点外关闭)必须与 ui/ 底件一致;Base UI 无独立 FocusScope(#158),ScrollShell 的内联焦点陷阱保留 `@radix-ui/react-focus-scope` 单点
